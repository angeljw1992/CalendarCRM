    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.tasks.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('grupo') ? 'is-invalid' : '' }}" name="grupo" id="grupo" required>
                    ...
                    @foreach(App\Models\Task::GRUPO_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('grupo', 'infantil') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('grupo'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('final_date') ? 'is-invalid' : '' }}" type="text" name="final_date" id="final_date" value="{{ old('final_date') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('dias') ? 'is-invalid' : '' }}" name="dias" id="dias" required>
                    ...
                    @foreach(App\Models\Task::DIAS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('dias', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('dias'))
